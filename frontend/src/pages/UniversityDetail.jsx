...
import {
  ...
  GitCompare, Clock, DollarSign, Home, Languages, GraduationCap, Award, ExternalLink,
} from "lucide-react";
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-zinc-500">
                ...
                {u.acceptance_rate != null && <span className="flex items-center gap-1"><GraduationCap className="h-4 w-4" />{u.acceptance_rate}% acceptance</span>}
              </div>
        ...
        {programs.map(({ program: p, fit, scholarships }) => (
          <div key={p.id} className="rounded-xl border border-zinc-200 bg-white p-5" data-testid={`program-${p.id}`}>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <Spec icon={GraduationCap} label="Min GPA" value={p.requirements?.min_gpa ? `${p.requirements.min_gpa.value}/4.0` : "Not published"} field={p.requirements?.min_gpa} />
